...
### First Fit
가장 먼저 최초로 발견되는 메모리공간에 할당한다.
### Best Fit
모든 빈 공간을 계산해서, 프로세스를 할당했을 때 가장 남는 공간이 적은(외부 단편화가 가장 적게 생길 확률이 높은)곳에 할당한다.
### Worst Fit(최악 적합)
남는 가장 큰 공간에 할당한다.

효율성은 속도와 메모리 효율 측면에서 First Fit과 Best Fit이 Worst Fit보다 좋은것으로 나타났습니다. 메모리 효율 측면에서 First Fit, Best Fit은 그다지 차이가 없는 것으로 나타났지만 속도는 당연히 First Fit이 빠른것으로 나타났다. First Fit은 초초로 공간이 보일 때할당하고, Best Fit은 모든 빈 공간을 계산해서 프로세스를 할당했을 때 가장 남는 공간이 적은 곳을 찾아내야 하기 때문이다.
